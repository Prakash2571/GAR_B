 ...
 *
 * GTS Box never performs its own Dhan consent flow. The Dhan access token is
 ...
 * configuration error, because it would begin a browser OAuth GTS Box must not
 * own.
 */
export async function generateDhanConsent(
  ...
  throw new DhanError(
    "Dhan consent login is disabled in GTS Box: the access token is provisioned by the CalSpread token provider, not by a Dhan consent flow.",
    400,
    "CONSENT_DISABLED",
    null,
  );
 ...
 * because GTS Box never initiates the browser login. Retained for the ported
