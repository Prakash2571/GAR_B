 ...
 *
 * It reads the predecessor codebase's Box collections from a legacy MongoDB (LEGACY_BOX_MONGODB_URI)
 * and writes them into this backend's authoritative PostgreSQL tables. It is the tool
 ...
 *     signs a GTS Algo Research process could be trading — an armed trading session, or an
 ...
 *    only have been produced by a live GTS Algo Research process.
 */
...
/** Raised when the target shows a GTS Algo Research process could be trading. */
export class LiveMarkersPresentError extends Error {
  readonly markers: readonly string[];
  constructor(markers: readonly string[]) {
    super(
      "Refusing to import while a GTS Algo Research process could be trading: " +
        `${markers.join("; ")}. Stop all GTS Algo Research processes and retry. ` +
