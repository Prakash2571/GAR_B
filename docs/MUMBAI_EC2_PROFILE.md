...
| `BOX_SESSION_MAX_COMPLETED_TRADES` | `1` (code default `0`) | Explicitly ARM a one-completed-box session; off unless set. Bounds SUCCESS only — see the next row. |
| `BOX_SESSION_MAX_ENTRY_ATTEMPTS` | `1` (code default `0` = **UNLIMITED**) | **The other half of "one trade", and previously MISSING from this profile.** A cycle is consumed only when a four-leg Box is ESTABLISHED, so an attempt that submits legs, partially fills and is unwound spends NO cycle — leaving this at its default permitted an unbounded run of failed attempts, each placing real orders. Consumed at ADMISSION before any broker POST, and durable across restart. Both ceilings are SNAPSHOTTED AT ARM: changing this requires a deliberate disarm/re-arm while FLAT. |
...
| `BOX_LIVE_REQUIRE_FUNDS_COVER` | `true` (code default `false`) | Refuse entry unless fresh broker funds cover the requirement. The requirement is now always a REAL margin-based figure: the old fallback that compared funds against the worst-case GROSS OPTION PREMIUM has been removed, because premium does not bound the margin a short option attracts. With no usable requirement, admission refuses `metric_incomplete`. |
| `BOX_LIVE_REQUIRE_MARGIN_EVIDENCE` | `true` (code default `false`) | Requires **COMPLETE** broker basket-margin evidence. Completeness is now enforced: the Dhan per-leg fallback used to return a sum over however many legs happened to answer, so a 1-of-4 sum was accepted as a basket margin. Every requested leg must resolve and return a structurally valid figure or the basket is unusable for admission. |
| `BOX_LIVE_REQUIRE_STAGE_FUNDING` | `true` (code default `false`) | Requires a defensible **EXECUTION-STAGE** funding requirement. The FINAL hedged margin is not evidence for the intermediate stages — while hedges are still going out the spread benefit does not exist yet (Zerodha's own basket example differs ~2.8x). **Zerodha: supported** (`data.initial.total` is documented as the execute-the-orders margin, and block PRESENCE is tracked separately from value). **Dhan: NOT ESTABLISHED** — no documented execute-the-orders field exists, `spanMargin` is a component and was previously mapped onto `initial` with `?? 0`, so an absent span became an established ₹0. It is now UNKNOWN, and **live entry on Dhan is BLOCKED with `funding_stage_unknown` under this profile**. That is intended; do not set this to `false` to get past it. |
